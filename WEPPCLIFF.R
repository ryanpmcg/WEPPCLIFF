...
print_license_agreement = function() {
  LICENSE = {"
  WEPP Climate File Formatter (WEPPCLIFF) Version 1.4
  ...
  McGehee, R.P., D.C. Flanagan, and P. Srivastava. 2019. WEPPCLIFF:
      ...
      Submitted to Journal of Open Source Software on 22 November 2019. 
...
assign_empty_args = function() {
  ...
  if (length(p) == 0){assign("p", plot.dir, envir = .GlobalEnv)} # Def: current directory
...
check_args = function() {
  ...
  ed.patterns = paste("0", "1", "2", "3", "4", "5", collapse = "|")
  ...
  if (grepl(ed, ed.patterns, ignore.case = T) != T){stop("Invalid Argument: [-ed] must be specified as '0' (off), '1' (rds), '2' (json), or '3-5' (precip, daily, or storm csv).")}
...
impose_daily_value_limits = function(data, dly.df) {
  ...
  d.l.lims = c(0, -90, 0, -90, -20, 0, 0, 0, -25)
...
start_of_year = function(date) {
  ...
  if (toupper(rtb) == "M") {if (month.start > real.start) {}}
...
end_of_year = function(date) {
  ...
  # Round the ending date based on user input.
  if (toupper(rtb) == "M") {if (month.end < real.end) {}}
...
export_data = function(data) {
  ...
  if (ed == 3){export_to_csv(data, 5, 15, "PRECIP")}
  if (ed == 4){export_to_csv(data, 10, 18, "DAILY")}
  if (ed == 5){export_to_csv(data, 9, 17, "STORM")}
...
generate_export_data = function(data, dly.loc, pcp.loc) {
  ...
  pcp_df$NON_PCP_TIME = pcp_df$ANT - pcp_df$DUR
  ...
  daily_df$BPS[increase][is.na(daily_df$BPS[increase])] = 0
  daily_df$BPS[increase] = daily_df$BPS[increase] + 1
  daily_df$BPS[decrease][is.na(daily_df$BPS[decrease])] = 0
  daily_df$BPS[decrease] = daily_df$BPS[decrease] - 1
  
  # Add a breakpoint for everyday there is precipitation.
  tmp = daily_df$BPS
  tmp[is.na(tmp) == T] = 0
  precip_days = tmp > 0
  daily_df$ADJ_BPS = daily_df$BPS
...
create_cli_file = function(data, header, export) {
  ...
  for (i in daily_rows){
    ...
    if (daily_df$BPS[i] > 0){
      ...
      temp.file = append(temp.file, temp.new)
      need.break = F
      last.pcp = current.pcp + daily_df$BPS[i] - 1
      ...
      for (j in current.pcp:last.pcp){
        ...
        non_pcp_time = pcp_df$ANT[j + 1] - pcp_df$DUR[j + 1]
...
core_workflow = function(file) {
  ...
  if (toupper(pd) == "T") {graph_data(data)}
...
flagnames = c("First Run", "License Agreement",
              "Input Directory", "Output Directory", "Export Directory", "Plot Directory", "Input Filename", "Output Filename", "File Delimiter",
...
package.list = c("backports", "crayon", "vctrs", "readr", "rlist", "iterators", "foreach", "doParallel", "EnvStats", "mice", "RcppParallel", "withr", "ggplot2", "profvis", "data.table")
par.pack.list = c("backports", "crayon", "vctrs", "readr", "rlist", "EnvStats", "mice", "withr", "ggplot2", "profvis", "data.table")
