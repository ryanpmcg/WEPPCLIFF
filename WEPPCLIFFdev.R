...
print_license_agreement = function() {
  ...
  This is free and open source software, and as the exclusive property of the
      copyright holder, it is licensed under the GNU GPLv3.

  \033[1mUNDER THIS LICENSE YOU CAN FREELY:\033[0m
      Execute, study, copy, or modify this software for your own personal use.

  \033[1mUNDER THIS LICENSE YOU CANNOT:\033[0m
...
assign_empty_args = function() {
  ...
  if (length(ee) == 0){assign("ee", "MM", envir = .GlobalEnv)} # Def: use the Brown Foster (BF) energy equation
...
create_cluster_col = function(in.df, dt.col.name, dt.format) {
  ...
  if (nrow(na.omit(in.df)) < 366) {stop("\033[1;31mInsufficient Data: at least one year of data must be present for reliable filling.\033[0m")}
